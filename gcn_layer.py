...
class HLAGraphConvolution(Module):
    ...
    def forward(self, input):
        ...
        if self.adj_op == 'content':
            x1 = self.trans_1(x)
            x2 = self.trans_2(x)
            g_sim = torch.matmul(x1, x2.permute(0, 2, 1))
...
class LAGCN2_Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_state_reshaped = torch.matmul(x_n_rel.permute(0, 2, 1), x_rproj_reshaped)
...
def generate_spatial_square(N=49):
    ...
    x_square = torch.from_numpy(x_square).float().cuda()
    x_square.requires_grad = False
    y_square = torch.from_numpy(y_square).float().cuda()
